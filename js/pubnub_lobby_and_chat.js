...
//
// Launch a chess game between the local user and the user that accepted the challenge.
...
//
//
// EXAMPLE: /chess.html?local_uuid=local_user&remote_uuid=remote_user&pubnub_chess_channel=pubnub_chess_channel_1&local_skill_level=beginner&chess_side=w
//
//  PARAMETERS:
//
//  remoteUserID            : the user ID of the opponent
//  chessSide               : the color/side selected for the game, White or Black ("w" or "b").
//
...
function navigateToChessGame(remoteUserID, chessSide)
{
    ...
    var pubnubChessChannel = private_channel;
    ...
    if (typeof remoteUserID == "undefined" || remoteUserID == null)
        throw new Error("(navigateToChessGame) The remote  user ID is unassigned.");

    if (typeof chessSide == "undefined" || chessSide== null)
        throw new Error("(navigateToChessGame) The selected side to play is unassigned.");

    if (chessSide != "w" && chessSide != "b")
        throw new Error("(navigateToChessGame) Only 'w' and 'b' are valid choices for the selected side to play (White or Black).");

    if (typeof localUserID == "undefined" || localUserID == null)
        throw new Error("(navigateToChessGame) The local user ID is unassigned.");

    if (typeof pubnubChessChannel == "undefined" || pubnubChessChannel == null)
        throw new Error("(navigateToChessGame) The PubNub chess channel ID is unassigned.");

    if (typeof localSkillLevel == "undefined" || localSkillLevel == null)
        throw new Error("(navigateToChessGame) The local user's skill level is unassigned.");
    ...
    var url =
        ...
        + "&"
        + "pubnub_chess_channel" + "=" + pubnubChessChannel
        + "&"
...
var channel = 'pubnub-chess-club-chat-lobby';
...
var Client = function () {
        ...
        pubnub.publish({
            ...
            callback: function () {
                ...
                if (response)
                    ...
                    navigateToChessGame(caller.uuid, "w");
            }
        });
    };
    ...
    self.on_response = function (caller, accepted) {
        if (accepted) {
            ...
            navigateToChessGame(caller.uuid, "b");
        } else {
